...
function getUserYeuCauMuon($maNguoiDung) {
    // Use lowercase table name to match database dump and avoid case issues on some systems
    $sql = "SELECT ycm.*, 
...
/**
 * Lấy tất cả yêu cầu mượn (dành cho admin)
 * @param int $limit Số bản ghi cần lấy (0 = tất cả)
 * @return array
 */
function getAllYeuCauMuon($limit = 0) {
    $sql = "SELECT ycm.*, nd_duyet.HoTen as TenNguoiDuyet, nd.YuHoTen as Dummy
            FROM `yeucaumuon` ycm
            LEFT JOIN `nguoidung` nd_duyet ON ycm.NguoiDuyet = nd_duyet.MaNguoiDung
